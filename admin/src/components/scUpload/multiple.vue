    ...
    <el-upload ref="uploader" list-type="picture-card"
      ...
      <template #file="{ file }">
        <div class="sc-upload-list-item">
          <el-image class="el-upload-list__item-thumbnail" :src="displayUrl(file)" fit="cover" :preview-src-list="preview" :initial-index="preview.findIndex(n=>n==displayUrl(file))" hide-on-click-modal append-to-body :z-index="9999">
...
<script>
  ...
  export default {
      ...
      modelValue(val){
        if(Array.isArray(val)){
          if (JSON.stringify(val) != JSON.stringify(this.formatArr(this.defaultFileList))) {
            this.defaultFileList = this.normalizeFileList(val)
            this.value = val
          }
        }else{
    ...
    computed: {
      preview(){
        return this.defaultFileList.map(v => this.displayUrl(v)).filter(Boolean)
      }
    },
    mounted() {
      this.defaultFileList = Array.isArray(this.modelValue) ? this.normalizeFileList(this.modelValue) : this.toArr(this.modelValue)
    ...
    methods: {
      ...
      toArr(str){
        ...
        arr.forEach(item => {
          if(item){
            _arr.push(this.formatFileItem(item))
          }
        })
        return _arr;
      },
      //数组转换为原始值
      toStr(arr){
        return arr.map(v => this.rawUrl(v)).filter(Boolean).join(",")
      },
      //格式化数组值
      formatArr(arr){
        var _arr = []
      ...
      normalizeFileList(arr){
        return arr.map(item => this.formatFileItem(item)).filter(item => item.url)
      },
      formatFileItem(item){
        const url = typeof item === 'string' ? item : this.rawUrl(item)
        if(!url){
          return {name: "", url: ""}
        }
        var urlArr = url.split('/');
        var fileName = typeof item === 'object' && item.name ? item.name : urlArr[urlArr.length - 1]
        return {
          ...(typeof item === 'object' ? item : {}),
          name: fileName,
          url,
          responseUrl: url,
          displayUrl: tool.resUrl(url)
        }
      },
      rawUrl(file){
        return (file?.responseUrl || file?.url || '').trim()
      },
      displayUrl(file){
        return file?.displayUrl || tool.resUrl(this.rawUrl(file))
      },
      //拖拽
      rowDrop(){
        const _this = this
      ...
      success(res, file){
        ...
        file.responseUrl = response.src
        file.displayUrl = tool.resUrl(response.src)
      },
      error(err){
        this.$notify.error({
          title: '上传文件未成功',
      ...
      handlePreview(uploadFile){
        window.open(this.displayUrl(uploadFile))
      },
      request(param){
